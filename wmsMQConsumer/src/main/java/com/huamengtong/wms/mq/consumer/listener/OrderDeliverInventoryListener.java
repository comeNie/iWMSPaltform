...
@Component
public class OrderDeliverInventoryListener extends ConsumeListener {
    ...
    public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext consumeConcurrentlyContext) {
        try {
            ...
            if(!mr.isSuc()){
                //如果分配失败,重新放入分配队列,10分钟后重新分配
                orderDeliverProducer.sendOrder(map,true, MqConstants.MQ_DELAY_LEVEL_10M);
            }
        } catch (Exception e) {
