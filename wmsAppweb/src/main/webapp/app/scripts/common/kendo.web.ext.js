...
define(['kendo','jquery'], function (kendo, $) {
    ...
    var ExtDialog = kendo.ui.Window.extend({
        _buttonTemplate: kendo.template('<div class="k-ext-dialog-buttons" style="position:absolute; bottom:10px; text-align:center; width:100%"><div style="display:inline-block"># $.each (buttons, function (idx, button) { # <button class="k-button" style="margin-right:5px;"><i class="fa #= button.className#"></i>#= button.name #</button> # }) # </div></div>'),
        _contentTemplate: kendo.template('<div class="k-ext-dialog-content" style="width:100%;overflow:auto;">'),
        ...
        init: function (element, options) {
            ...
            options.actions = options.actions || ["Close"];
            options.activate = function(){
              var button = $($(element).parent().find(".k-ext-dialog-buttons .k-button")[0]);
              button.focus();
    ...
    kendo.ui.ExtAlertDialog = {
        showError : function(message, targetEl) {
          if (targetEl && targetEl.length > 0) {
            return $.when(kendo.ui.ExtAlertDialog.show({
              title: "错误",
              message: message,
              icon: 'k-ext-error' })).then(function(){
              setTimeout(function () {
                targetEl.focus();
              },500);
            });
        ...
        show: function (options) {
            return new $.Deferred(function (deferred) {
                ...
                $(document.body).append(kendo.format("<div id='extAlertDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='{0}'></div><div style='display:inline-block;width: 100%;text-align: center;padding-top:10px'>{1}</div></div>", options.icon, options.message));
    ...
    kendo.ui.ExtOkCancelDialog = {
        show: function (options) {
            return new $.Deferred(function (deferred) {
                ...
                $(document.body).append(kendo.format("<div id='extOkCancelDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='{0}'></div><div style='display:inline-block;width: 100%;text-align: center;padding-top: 10px'>{1}</div></div>", options.icon, options.message));
    ...
    kendo.ui.ExtYesNoDialog = {
        show: function (options) {
            return new $.Deferred(function (deferred) {
                ...
                $(document.body).append(kendo.format("<div id='extYesNoDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='{0}'></div><div style='display:inline-block;width:100%;text-align: center'>{1}</div></div>", options.icon, options.message));
    ...
    kendo.ui.ExtWaitDialog = {
        show: function (options) {
            return new $.Deferred(function (deferred) {
                ...
                $(document.body).append(kendo.format("<div id='extWaitDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='k-ext-wait'></div><div style='display:inline-block;width:100%;text-align: center;padding-top:10px'>{0}</div></div>", options.message));
        ...
        hide: function () {
          if ($("#extWaitDialog").data("kendoWindow")) {
